      ...
      <div style={{ padding: '6rem 5% 4rem', maxWidth: '800px', margin: '0 auto' }}>
        ...
        {id === 'general' ? (
          <div style={{ marginBottom: '3rem', background: 'linear-gradient(135deg, var(--primary) 0%, #db6a51 100%)', padding: '3rem', borderRadius: '30px', boxShadow: '0 20px 50px rgba(188, 73, 49, 0.2)', color: 'white', textAlign: 'center' }}>
            <h1 style={{ fontSize: '2.5rem', marginBottom: '1rem', fontFamily: "'Playfair Display', serif" }}>Discussion Générale</h1>
            <p style={{ opacity: 0.9, fontSize: '1.1rem' }}>Partagez vos impressions sur AL-RIAD et Marrakech en général.</p>
          </div>
        ) : place && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '2rem', marginBottom: '3rem', background: 'white', padding: '1.5rem', borderRadius: '20px', boxShadow: '0 10px 30px rgba(0,0,0,0.05)' }}>
            ...
            <div>
              <h1 style={{ fontSize: '1.5rem', marginBottom: '0.5rem' }}>Commentaires : {place.title}</h1>
        ...
        <div style={{ background: 'white', borderRadius: '25px', padding: '2rem', boxShadow: '0 20px 50px rgba(0,0,0,0.05)' }}>
          <form onSubmit={handleSubmit} style={{ marginBottom: '3rem' }}>
            <h3 style={{ marginBottom: '1.2rem', fontSize: '1.1rem' }}>Partagez votre avis</h3>
            <div style={{ position: 'relative' }}>
              <textarea
                value={newComment}
                onChange={(e) => setNewComment(e.target.value)}
                placeholder={user ? "Écrivez un commentaire..." : "Connectez-vous pour laisser un commentaire"}
                style={{ 
            ...
            {!user && (
               <p style={{ fontSize: '0.85rem', color: '#888', marginTop: '0.8rem' }}>
                 Vous devez être <span onClick={() => navigate('/login')} style={{ color: 'var(--primary)', cursor: 'pointer', fontWeight: '600' }}>connecté</span> pour commenter.
               </p>
            )}
          </form>
